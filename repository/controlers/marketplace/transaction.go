...
func GetTransactionsByBuyerId(buyer_id int, dbPool *pgxpool.Pool) ([]models.Transaction, error) {
	...
	query := `SELECT id, final_price, transaction_time, transaction_type, buyer_id, seller_id, listing_id, payment_status, payment_due_time FROM marketplace.transaction WHERE buyer_id = $1`
